...
TEST(SparseTensorConversionTests, CsrConversion) {
  ...
  {
    auto cpu_transfer = std::make_unique<CPUDataTransfer>();
    dtm.RegisterDataTransfer(std::move(cpu_transfer));
  }
 
  {
    // test where both src and destination are on CPU
    Tensor cpu_src(DataTypeImpl::GetType<int32_t>(), dense_shape, dense_data.data(), cpu_allocator->Info());
    SparseTensor dst;
    ASSERT_STATUS_OK(sparse_utils::DenseTensorToSparseCsr(dtm, cpu_src, cpu_allocator, cpu_allocator, dst));
    ASSERT_EQ(cpu_src.DataType(), dst.DataType());
